...
use Ticoseo\Common\Traits\Requester;
use Ticoseo\Common\Utils\Errors;
use Ticoseo\Config\Classes;
use Ticoseo\Config\I18n;
use Ticoseo\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'ticoseo_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'ticoseo_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'ticoseo' ),
					__FILE__, $err
				);
			}
		}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'TicoSEO classes are not being loaded by Composer\'s Autoloader' ),
